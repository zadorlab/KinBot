...
def get_frequencies(species, hess, natom, atom, geom):
    ...
    all_freqs = [convert_to_wavenumbers(ei) for ei in sorted(all_eigvals)]
    ...
    for i in range(3):
        tvecs[i] = np.roll([np.array([1., 0., 0.] * natom) * np.sqrt(masses)], i)
    ...
    for rot in species.dihed:
        ...
        for at in range(natom):
            ...
            per = vect - proj #vector perpendicular to the rotational axis through the atom
        ...
        #project the translational, external rotational and previous internal rotations
        #out of the current vector
