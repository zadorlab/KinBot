                        ...
                        if self.shigh == 0:
                            ...
                            if status == 'normal':
                                ...
                                if geometry.equal_geom(self.species.bond,self.species.geom,new_geom,0.1):
                                    ...
                                    err, self.freq = self.qc.get_qc_freq(self.job_high, self.species.natom)
                                    err, self.zpe = self.qc.get_qc_zpe(self.job_high)
                                    self.shigh = 1
                                else:
                                    ...
                                    if len(self.species.hir.hir_energies) > 0:
                                        ...
                                        for rotor in range(len(self.species.dihed)):
                                            for ai in range(self.species.hir.nrotation):
                                                if self.species.hir.hir_energies[rotor][ai] < min - 1.6E-4: #use a 0.1kcal/mol cutoff for numerical noise 
