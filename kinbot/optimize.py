...
from kinbot.conformer_records import update_member, hessian_record
...
class Optimize:
    ...
    def __init__(self, species, par, qc, wait=0, just_high=False):
        ...
        self.species.conformer_representation = ('MC-RRHO' if par.get('multi_conf_tst', 0)
                                                else 'selected conformer')
        self.species.optical_population = par.get('optical_population', 'specified')
        ...
        if self.species.wellorts or par['cluster']:
            ...
            self.name = routing_name(self.species)
        if not self.species.wellorts:
            optical_scope(self.species, self.species.optical_population)
        from kinbot.optical import bind_assumption
        bind_assumption(self.species, par)
        self.par = par
        self.qc = qc
    ...
    def do_optimization(self):
        ...
        try:
            return self._do_optimization()
        except StereoRoutingError as error:
            # A mismatched configured cache invalidates this optimization,
            # not independent reactions already generated by KinBot.
            logger.warning('%s: rejecting this optimization because %s', self.name, error)
            self.shigh = -999
            return 0
                    ...
                    if self.par['high_level'] == 1 or self.restart > 0:
                        if self.shigh == -1:
                            if not self.par['high_level']:
                                ...
                                if self.just_high:
                                    ...
                                    name = routing_name(self.species)
                        ...
                        if self.shigh == 0.5:  # the top one was tested already and was ok
                            ...
                            for ci, conindx in enumerate(self.species.conformer_index):
                                ...
                                if status == 'error':
                                    stati[ci] = 1
                                    update_member(self.species, conindx, accepted=False,
                                                  source_job=self.log_name(1, conf=conindx))
                        ...
                        if (self.shigh == -1 and not self.par['conformer_search']
                                and not self.just_high):
                            # No later optimization supplies the L1 properties.
                            # Load them together before HIR/projection; product
                            # objects can still have an empty raw frequency list.
                            load_calculation_record(self.species, self.qc,
                                                    selected_calculation_job(self))
                            ...
                            if self.shir == 0:
                                ...
                                if status:  # Finished correctly
                                    if len(self.species.hir.hir_energies) > 0:
                                        ...
                                        if min_rotor > -1:
                                            self.restart += 1
                                            job = self.species.hir.point_job(min_rotor, min_ai)
                                            ...
                                            if self.restart < self.par['rotation_restart']:
                                                ...
                                                for rotor in range(len(self.species.dihed)):
                                                    for ai in range(self.species.hir.nrotation):
                                                        self.qc.invalidate_qc(self.species.hir.point_job(rotor, ai))
            ...
            if self.shir == 1:
                ...
                if (self.par['multi_conf_tst'] == 0
                    ...
                    if not recover_hir_model(self.species, self.qc, self.par):
                        ...
                        self.species.hessian_source_job = fr_file
                        self.species.kinbot_freqs, self.species.reduced_freqs = frequencies.get_frequencies(
                            self.species, hess, self.species.geom, massweighted=massweighted)
                else:
    ...
    def compare_structures(self, conf=-1, job=None):
        ...
        if self.species.wellorts and job == getattr(self, '_hir_refinement_job', None):
            ...
            same_geom = same_geom and configured_geometry_allowed(self.species, new_geom)

        # An annotated TS conformer must remain in this route's ensemble.
        if self.species.wellorts:
            same_geom = same_geom and configured_geometry_allowed(self.species, new_geom)
        ...
        if conf == -1:
            ...
            if same_geom and freq_ok:
                load_calculation_record(self.species, self.qc, job)
            ...
            if same_geom and freq_ok:
                ...
                update_member(self.species, conf, source_job=job,
                    hessian_data=hessian_record(self.qc, job,
                        self.species.conformer_geom[inx], self.species.atom))
            else:
                update_member(self.species, conf, accepted=False,
                              source_job=job)
