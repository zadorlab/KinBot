...
class S12ShiftR(GeneralReac):
    ...
    def get_constraints(self,step, geom):
        ...
        if step < self.max_step:
            ...
            self.set_bond(1, 2, -999, change, step=step, stmax=self.step_max, findist=final_dist1, geom=geom)
            
            final_dist2 = constants.st_bond[''.join(sorted(self.species.atom[self.instance[0]]+self.species.atom[self.instance[2]]))]
            self.set_bond(0, 2, -999, change, step=step, stmax=self.step_max, findist=final_dist2, geom=geom)
