...
def do_Intra_RH_Add_Exocyclic_F(species, instance, step, instance_name):
    """
    ...
    step, fix, change, release = get_constraints_Intra_RH_Add_Exocyclic_F(step, species, instance,geom)
...
def get_constraints_Intra_RH_Add_Exocyclic_F(step,species,instance,geom):
    """
    ...
    if step < 12:
        ...
        val = new_bond_length(species,instance[1],instance[-2],step - 11,10,final_dist[0],geom)
        constraint = [instance[1] + 1,instance[-2] + 1,val]
        change.append(constraint)
        
        val = new_bond_length(species,instance[-1],instance[-2],step - 11,10,final_dist[1],geom)
        constraint = [instance[-1] + 1,instance[-2] + 1,val]
        change.append(constraint)
        
        val = new_bond_length(species,instance[0],instance[-1],step - 11,10,final_dist[2],geom)
        constraint = [instance[0] + 1,instance[-1] + 1,val]
        change.append(constraint)
        
        val = new_bond_length(species,instance[0],instance[1],step - 11,10,final_dist[3],geom)
        constraint = [instance[0] + 1,instance[1] + 1,val]
        change.append(constraint)
        
        val = new_bond_length(species,instance[-1],instance[1],step - 11,10,final_dist[4],geom)
        constraint = [instance[-1] + 1,instance[1] + 1,val]
        change.append(constraint)
