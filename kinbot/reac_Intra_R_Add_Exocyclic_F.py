...
def do_Intra_R_Add_Exocyclic_F(species, instance, step, instance_name):
    """
    ...
    skip = 1 #skip the first 12 steps if instance has less than 4 elements
    step,geom, kwargs = initialize_reaction(species, instance, step, instance_name, max_step, skip)

    #the the constraints for this step
    step, fix, change, release = get_constraints_Intra_R_Add_Exocyclic_F(step, species, instance,geom)
...
def get_constraints_Intra_R_Add_Exocyclic_F(step,species,instance,geom):
    """
    ...
    if step < 12:
        ...
        for dih in range(len(instance)-4): #do not use the last atom
            constraint = []
            for i in range(4):
        ...
        fval = 2.2
        constraint = [instance[0] + 1,instance[-2] + 1,fval]
        change.append(constraint)
