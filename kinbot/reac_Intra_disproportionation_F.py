...
def do_Intra_disproportionation_F(species, instance, step, instance_name):
    """
    ...
    step, fix, change, release = get_constraints_Intra_disproportionation_F(step, species, instance,geom)
...
def get_constraints_Intra_disproportionation_F(step,species,instance,geom):
    """
    ...
    if step < 12:
        ...
        val = new_bond_length(species,instance[-2],instance[-1],step-11,10,fval,geom)
        constraint = [instance[-2] + 1,instance[-1] + 1,val]
        change.append(constraint)
