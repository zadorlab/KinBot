...
def do_cpd_H_migration(species, instance, step, instance_name):
    """
    ...
    step, fix, change, release = get_constraints_cpd_H_migration(step, species, instance,geom)
...
def get_constraints_cpd_H_migration(step,species,instance,geom): 
    """
    ...
    if step == 0:
        constraint = [(instance[i] + 1) for i in range(-2,1)]
        constraint.append(70.)
        change.append(constraint)
    if step == 1:
        fval = 1.35
        constraint1 = [instance[-2] + 1,instance[-1] + 1,fval]
        change.append(constraint1)
        
        constraint2 = [instance[0] + 1,instance[-1] + 1,fval]
