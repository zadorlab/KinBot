...
def do_r12_cycloaddition(species, instance, step, instance_name):
    """
    ...
    step, fix, change, release = get_constraints_r12_cycloaddition(step, species, instance,geom)
...
def get_constraints_r12_cycloaddition(step,species,instance,geom):
    """
    ...
    if step < par.scan_step:
        ...
        val = np.linalg.norm(geom[instance[0]] - geom[instance[2]]) + 0.02
        constraint = [instance[0] + 1,instance[2] + 1,val]
        change.append(constraint)
        
        val = np.linalg.norm(geom[instance[1]] - geom[instance[2]]) + 0.02
