...
class VrcTstScan(GeneralReac):
    def __init__(self, species, qc, par, instance, instance_name, scan_type='trust'):
        ...
        self.frozen_param = [[index + 1 for index in self.instance]]  # fixing the scanned bond
        ...
        while fragments_optimized < len(fragments) or hl_opt < len(fragments):
            ...
            for frag_number, frag in enumerate(fragments):
                ...
                if err == 0:
                    ...
                    if scan_type=='sample':
                        self.set_frozen_coord(level='L1', frag=frag, frag_number=frag_number)  # Must only be called once before start of the scan
                ...
                if self.par['high_level']:
                    ...
                    if err == 0:
                        ...
                        if scan_type=='sample':
                            self.set_frozen_coord(level='L2', frag=frag, frag_number=frag_number)  # Must only be called once before start of the scan
                        hl_opt += 1
                        #Save the cartesian coordinates
    ...
    def set_frozen_coord(self, level='L1', frag=None, frag_number=None):
        ...
        frag.find_bond()
        for fb in frag.bondlist:
            bond_length = frag.dist[fb[0]][fb[1]]
            self.long_range['coord'][level][f'{frag_number}'].append([self.long_range['maps'][frag_number][fb[0]],
                                                                      self.long_range['maps'][frag_number][fb[1]],
                                                                      bond_length])
            if level == 'L1':
                self.frozen_param.append([self.long_range['maps'][frag_number][fb[0]]+1,
                                          self.long_range['maps'][frag_number][fb[1]]+1])

        # Save the angles
        frag.find_angle()
        collinear_cutoff = 179.0
        for fa in frag.angle:
            point_A = frag.geom[fa[0]]
            point_B = frag.geom[fa[1]]
            point_C = frag.geom[fa[2]]
            angle_value = np.degrees(geometry.calc_angle(point_A, point_B, point_C))
            if level == 'L1':
                if angle_value < collinear_cutoff:
                    self.frozen_param.append([self.long_range['maps'][frag_number][fa[0]] + 1,
                                              self.long_range['maps'][frag_number][fa[1]] + 1,
                                              self.long_range['maps'][frag_number][fa[2]] + 1])
                    self.long_range['coord'][level][f'{frag_number}'].append([self.long_range['maps'][frag_number][fa[0]],
                                                                             self.long_range['maps'][frag_number][fa[1]],
                                                                             self.long_range['maps'][frag_number][fa[2]],
                                                                             angle_value])
                else:
                    self.frozen_param.append(['L',
                                              self.long_range['maps'][frag_number][fa[0]] + 1,
                                              self.long_range['maps'][frag_number][fa[1]] + 1,
                                              self.long_range['maps'][frag_number][fa[2]] + 1,
                                              0])
                    self.long_range['coord'][level][f'{frag_number}'].append(['L', 
                                                                             self.long_range['maps'][frag_number][fa[0]],
                                                                             self.long_range['maps'][frag_number][fa[1]],
                                                                             self.long_range['maps'][frag_number][fa[2]],
                                                                             -1,
                                                                             angle_value])
        ...
        #Save the dihedrals
        frag.find_alldihedral(collinear_cutoff=collinear_cutoff)
        for fd in frag.dihed_all:
            point_A = frag.geom[fd[0]]
            point_B = frag.geom[fd[1]]
            point_C = frag.geom[fd[2]]
            point_D = frag.geom[fd[3]]
            dihedral_value = geometry.calc_dihedral(point_A, point_B, point_C, point_D)[0]
            self.long_range['coord'][level][f'{frag_number}'].append([self.long_range['maps'][frag_number][fd[0]],
                                                                  self.long_range['maps'][frag_number][fd[1]],
                                                                  self.long_range['maps'][frag_number][fd[2]],
                                                                  self.long_range['maps'][frag_number][fd[3]],
                                                                  dihedral_value])
            if level == 'L1' and 'frozen' in self.instance_name:
                self.frozen_param.append([self.long_range['maps'][frag_number][fd[0]]+1,
                                          self.long_range['maps'][frag_number][fd[1]]+1,
                                          self.long_range['maps'][frag_number][fd[2]]+1,
                                          self.long_range['maps'][frag_number][fd[3]]+1])
        ...
        #Additional check to see if this is the correct vdW well
            ...
            for frag_chemid, prod_chemid in zip(sorted(frag_chemids), sorted(prod_chemids)):
                if frag_chemid != prod_chemid:
                    logger.info(f'Cancelling scan {self.instance_basename}: products in db and in input are different.')
