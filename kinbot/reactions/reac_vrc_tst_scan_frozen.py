...
class VrcTstScanFrozen(VrcTstScan):
    ...
    def get_frozen_species(self, level="L1", distance=None):
        ...
        for modif in reversed(interfragments_param):
            match len(modif):
                case 5:
                    ...
                    point_A = geometries[frag_number[0]][np.where(self.frozen["maps"][frag_number[0]] == modif[0])[0]][0]
                    point_B = geometries[frag_number[1]][np.where(self.frozen["maps"][frag_number[1]] == modif[1])[0]][0]
                    point_C = geometries[frag_number[2]][np.where(self.frozen["maps"][frag_number[2]] == modif[2])[0]][0]
                    point_D = geometries[frag_number[3]][np.where(self.frozen["maps"][frag_number[3]] == modif[3])[0]][0]
        ...
        point_A = geometries[frag_number[0]][np.where(self.frozen["maps"][frag_number[0]] == self.instance[0])[0]][0]
        point_B = geometries[frag_number[1]][np.where(self.frozen["maps"][frag_number[1]] == self.instance[1])[0]][0]
