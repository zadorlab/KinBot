...
with open('fairchem.log', 'a') as f:
    f.write('Starting IRC Calculation {label}\n')
...
try:
    ...
    if converged_irc:
        ...
        with open('fairchem.log', 'a') as f:
            f.write('Continuing from final geometry {label}\n')
        forces = mol.calc.results['forces']
        del mol.calc.results['forces']
        random.seed()
    ...
    if mol.positions is not None and mol.positions.any():
        # although there is an error, continue from the final geometry
        with open('fairchem.log', 'a') as f:
            f.write('Contuining from final geometry_2 {label}\n')
        forces = mol.calc.results['forces']
        del mol.calc.results['forces']
        random.seed()
        db.write(mol, name='{label}', data={{'forces': forces, 'status': 'normal'}})
...
if success:
    ...
    try:
        ...
        if converged_opt:
            with open('fairchem.log', 'a') as f:
                f.write('IRC Optimization Converged! {label}\n')
        ...
        with open('fairchem.log', 'a') as f:
            f.write('IRC optimization did not converge {label}\n')
        forces = mol.calc.results['forces']
        del mol.calc.results['forces']
        random.seed()
