...
with open('fairchem.log', 'a') as f:
    f.write('Calculated initial energy. Beginning optimization {label}\n')
...
if len(mol) == 1:
    with open('fairchem.log', 'a') as f:
        f.write('Detected monatomic well {label}. Calculating energy instead of optimization.\n')
...
if True:
    ...
    while not converged and attempts <= 3:
        ...
        with open('fairchem.log', 'a') as f:
            f.write(f'Starting attempt {{attempts}} {label}\n')
        try:
            converged = opt.run(fmax=fmax, steps=steps)
        except ValueError:
            with open('fairchem.log', 'a') as f:
                f.write(f'Initial optimization failed. Perturbing coordinates. {label}\n')
        ...
        if order == 0 and (np.count_nonzero(np.array(freqs) < 0) > 1
            ...
            if attempts <= 3:
                print(f'Retrying with a tighter criterion: fmax={{fmax}}.')
                with open('fairchem.log', 'a') as f:
                    f.write('Retrying with a tighter criterion {label}\n')

        else:
            converged = True
            with open('fairchem.log', 'a') as f:
                f.write('Completed optimization! Updating kinbot.db {label}\n')
            e = mol.get_potential_energy()
            forces = mol.calc.results['forces']
            del mol.calc.results['forces']
            random.seed()
