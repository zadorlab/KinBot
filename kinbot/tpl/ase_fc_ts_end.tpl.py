...
with open('fairchem.log', 'a') as f:
    f.write('Optimizing transition state {label}\n')
...
try:
    ...
    while not converged and attempts <= 3:
        ...
        if (np.count_nonzero(np.array(freqs) < 0) > 2  # More than two imag frequencies
            ...
            with open('fairchem.log', 'a') as f:
                f.write('Finished optimization of transition state. Writing in kinbot.db {label}\n')
            random.seed()
            db.write(mol, name='{label}', 
                     data={{'energy': e, 'frequencies': freqs, 'zpe': zpe, 
                         'hess': hessian, 'forces': forces, 'status': 'normal'}})            
            with open('fairchem.log', 'a') as f:
                f.write('Completed kinbot.db for TS optimization {label}\n')
    ...
    with open('fairchem.log', 'a') as f:
        f.write('TS optimization failed {label}\n')
