...
with open('fairchem.log', 'a') as f:
    f.write('Starting transition state search {label}\n')
...
for fix in base_0_fix:
    if len(fix) == 2:
        ...
        with open('fairchem.log', 'a') as f:
            f.write('Transition state search failed (1) {label}\n')
...
try:
    ...
    if cvgd:
        ...
        with open('fairchem.log', 'a') as f:
            f.write('Transition state search did not converge {label}\n')
        raise RuntimeError
...
with open('fairchem.log', 'a') as f:
    f.write('Success! Writing in kinbot.db now for {label}\n')
...
with open('fairchem.log', 'a') as f:
    f.write('Completed transition state search {label}\n')
